...
(function() {
  ...
  if (typeof AbortController === 'undefined') {
    class AbortSignal { #listeners = {}; aborted = false; reason = undefined;
      ...
      dispatchEvent(ev) { for (const fn of (this.#listeners[ev.type] || []).slice()) { fn(ev); if (ev._stopImmediate) break; } return !ev.defaultPrevented; }
      _abort(reason) { if (this.aborted) return; this.aborted = true; this.reason = reason || new DOMException('The operation was aborted.', 'AbortError'); if (this.onabort) this.onabort(); const ev = new Event('abort'); ev.target = ev.currentTarget = this; this.dispatchEvent(ev); }
  ...
  if (typeof Event === 'undefined') {
    globalThis.Event = class Event { constructor(type, opts) { if (arguments.length === 0) { const e = new TypeError('Failed to construct \'Event\': 1 argument required, but only 0 present.'); e.code = 'ERR_MISSING_ARGS'; throw e; } if (typeof type === 'symbol') throw new TypeError('Cannot convert a Symbol value to a string'); this.type = String(type); this.bubbles = opts?.bubbles || false; this.cancelable = opts?.cancelable || false; this.composed = opts?.composed || false; this.defaultPrevented = false; this.target = null; this.currentTarget = null; this.eventPhase = 0; this.isTrusted = false; this.timeStamp = (typeof performance !== 'undefined' && performance.now) ? performance.now() : 0; } preventDefault() { if (this.cancelable) this.defaultPrevented = true; } stopPropagation() { this._stopPropagation = true; } stopImmediatePropagation() { this._stopPropagation = true; this._stopImmediate = true; } composedPath() { return this.target ? [this.target] : []; } get returnValue() { return !this.defaultPrevented; } set returnValue(v) { if (v === false) this.preventDefault(); } get cancelBubble() { return !!this._stopPropagation; } set cancelBubble(v) { if (v) this._stopPropagation = true; } get srcElement() { return this.target; } get NONE() { return 0; } get CAPTURING_PHASE() { return 1; } get AT_TARGET() { return 2; } get BUBBLING_PHASE() { return 3; } static get NONE() { return 0; } static get CAPTURING_PHASE() { return 1; } static get AT_TARGET() { return 2; } static get BUBBLING_PHASE() { return 3; } };
  ...
  if (typeof EventTarget === 'undefined') globalThis.EventTarget = class EventTarget { #h = {}; addEventListener(t, listener, opts) { if (listener == null) return; const signal = opts && opts.signal; if (signal !== undefined && !(signal instanceof globalThis.AbortSignal)) throw new TypeError('The "options.signal" property must be an instance of AbortSignal. Received ' + typeof signal); if (signal && signal.aborted) return; const arr = (this.#h[t] ??= []); if (arr.some(e => e.listener === listener)) return; /* WHATWG EventTarget dedups identical listeners */ arr.push({ listener, once: !!(opts && opts.once) }); if (signal) signal.addEventListener('abort', () => this.removeEventListener(t, listener)); } removeEventListener(t, listener) { const a = this.#h[t]; if (a) { const i = a.findIndex(e => e.listener === listener); if (i >= 0) a.splice(i, 1); } } listenerCount(t) { return (this.#h[t] || []).length; } _eventListeners(t) { return (this.#h[t] || []).map(e => e.listener); } dispatchEvent(ev) { ev.target = this; ev.currentTarget = this; ev.eventPhase = 2; const list = (this.#h[ev.type] || []).slice(); for (const entry of list) { const cur = this.#h[ev.type]; if (!cur || cur.indexOf(entry) === -1) continue; /* skip listeners removed mid-dispatch, e.g. signal.abort() fired by an earlier listener */ if (entry.once) this.removeEventListener(ev.type, entry.listener); if (typeof entry.listener === 'function') { entry.listener.call(this, ev); } else if (typeof entry.listener === 'object' && entry.listener !== null && typeof entry.listener.handleEvent === 'function') { entry.listener.handleEvent(ev); } if (ev._stopImmediate) break; } return !ev.defaultPrevented; } };
  ...
  if (typeof URL === 'undefined') {
    // application/x-www-form-urlencoded codec (WHATWG): '+' is space; percent
    // sequences are decoded leniently (invalid '%xx' kept verbatim, unlike
    ...
    // Lazy — TextEncoder/TextDecoder globals are registered later in bootstrap.
    let _formEnc, _formDec;
    const _enc = () => (_formEnc ??= new TextEncoder());
    const _dec = () => (_formDec ??= new TextDecoder());
    function _formDecode(s) {
      s = s.replace(/\+/g, ' ');
      const bytes = [];
      for (let i = 0; i < s.length; i++) {
        ...
        } else { for (const b of _enc().encode(c)) bytes.push(b); }
      }
        ...
        if (typeof init === 'string') {
          ...
          if (typeof init[Symbol.iterator] === 'function') {
            for (const pair of init) {
              const a = [...pair];
              if (a.length !== 2) throw _makeNodeError(TypeError, 'ERR_INVALID_TUPLE', 'Each query pair must be an iterable [name, value] tuple');
              this.#p.push([String(a[0]), String(a[1])]);
            }
          } else { for (const k of Object.keys(init)) this.#p.push([String(k), String(init[k])]); }
        }
      }
  ...
  if (typeof CustomEvent === 'undefined') globalThis.CustomEvent = class CustomEvent extends Event { constructor(type, opts) { if (arguments.length === 0) { const e = new TypeError('Failed to construct \'CustomEvent\': 1 argument required, but only 0 present.'); e.code = 'ERR_MISSING_ARGS'; throw e; } if (opts !== undefined && opts !== null && typeof opts !== 'object') { const e = new TypeError(`The "options" argument must be of type object. Received type ${typeof opts} (${opts})`); e.code = 'ERR_INVALID_ARG_TYPE'; throw e; } super(type, opts); this._detail = opts?.detail ?? null; } get detail() { return this._detail; } get [Symbol.toStringTag]() { return 'CustomEvent'; } };
  ...
  globalThis._ERR_INVALID_THIS = function(type) {
    return _makeNodeError(TypeError, 'ERR_INVALID_THIS', `Value of "this" must be of type ${type}`);
  };
  globalThis._ERR_MISSING_ARGS = function(...args) {
    const q = args.map(a => `"${a}"`);
    let list;
    ...
    return _makeNodeError(TypeError, 'ERR_MISSING_ARGS', `The ${list} ${noun} must be specified`);
  };
