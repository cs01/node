...
| net        | 57/106    | 54%  | high     | **MEASURED 2026-07-16.** pre-session 44 (oom 3, timeout 12). session: **44->56, oom 3->0, timeout 12->4**. Socket DOES extend Duplex already — that blocker is stale. dns-lookup lever DONE. remaining 4 timeouts: write-slow, listen-fd0, local-address, connect-options-port |
...
| http2      | 43/165    | 26%  | low      | **MEASURED 2026-07-16.** frame codec + HPACK exist and the round-trip matches the oracle byte-for-byte — the gap is missing surface, not a broken protocol. :method/:authority/:scheme defaults FIXED. setTimeout stubs FIXED (Http2Stream + Http2Session roots; Request/Response delegate). next: writeEarlyHints, and 33 timeouts to triage — each is one live handle + one missing js event (auditor's live sampling) |
