...
| net        | 61/106    | 58%  | high     | **MEASURED 2026-07-17.** pre-session 44 (oom 3, timeout 12). session: **44->61, oom 3->0, timeout 12->1**. Socket DOES extend Duplex already — that blocker is stale. dns-lookup lever DONE. end()-before-connect defers FIN to 'connect'. listen({fd}) adopts the fd (fd 0 -> EINVAL). socket-reconnect: end() latches reset on _undestroy. connect() now surfaces the sync errno via nm_connect (connect(0) -> EADDRNOTAVAIL instead of hanging). remaining 1 timeout: write-slow (fcntl-variadic blocking, §5e#1 — needs EAGAIN backpressure) |
