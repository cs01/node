...
| net        | 59/106    | 56%  | high     | **MEASURED 2026-07-17.** pre-session 44 (oom 3, timeout 12). session: **44->59, oom 3->0, timeout 12->3**. Socket DOES extend Duplex already — that blocker is stale. dns-lookup lever DONE. end()-before-connect defers FIN to 'connect'. listen({fd}) now adopts the fd (was ignored -> bogus random-port success); fd 0 -> EINVAL. remaining 3 timeouts: write-slow, local-address, connect-options-port |
