...
| http       | 93/210    | 44%  | high     | **MEASURED 2026-07-16, HONEST COUNT.** pre-session 79 (oom 11, timeout 40). now **93, oom 0, timeout 18** — honest, unlike the mid-session 87 which included ~5 false passes from swallowed handler exceptions. next: 100-continue (checkContinue/writeContinue absent), keepAliveTimeout/maxRequestsPerSocket advertised but not ENFORCED (playbook 5f), header-parse clientError |
