...
| http       | 92/210    | 44%  | high     | **MEASURED 2026-07-16, HONEST COUNT.** pre-session 79 (oom 11, timeout 40). now **92, oom 0, timeout 19** — honest, unlike the mid-session 87 which included ~5 false passes from swallowed handler exceptions. next: 100-continue (checkContinue/writeContinue absent), keepAliveTimeout/maxRequestsPerSocket advertised but not ENFORCED (playbook 5f), header-parse clientError |
| net        | 52/106    | 49%  | high     | **MEASURED 2026-07-16.** pre-session 44 (oom 3, timeout 12). session: **44->52, oom 3->0, timeout 12->6**. Socket DOES extend Duplex already — that blocker is stale |
