...
- assert REMAINING (each its own focused effort): typedarray/ArrayBuffer/SharedArrayBuffer deepEqual + partialDeepStrictEqual semantics (no TypedArray branch in `_deepEqual`, relies on slow Object.keys; SharedArrayBuffer inspect-labels as "ArrayBuffer"); error-cause deep compare (test-assert-deep-with-error); Symbol.toStringTag in deep-equal (test-assert-checktag).
