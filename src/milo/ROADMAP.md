...
| http       | 99/210    | 47%  | high     | **MEASURED 2026-07-17, HONEST COUNT.** pre-session 79 (oom 11, timeout 40). now **99, oom 1 (pre-existing, outgoing-finish, A/B-confirmed), timeout 15**. server keep-alive fixed: ServerResponse.end() no longer FINs the socket on keep-alive (http.js:413) — the socket stays open for reuse, express keepAlive Agent verified. next: chunked-vs-Content-Length for res.end(body), maxRequestsPerSocket ENFORCEMENT (advertised not enforced, playbook 5f), 100-continue, header casing |
| net        | 62/106    | 58%  | high     | **MEASURED 2026-07-17. ZERO TIMEOUTS.** pre-session 44 (oom 3, timeout 12). session: **44->62, oom 3->0, timeout 12->0**. Socket DOES extend Duplex already — that blocker is stale. dns-lookup lever DONE. end()-before-connect defers FIN to 'connect'. listen({fd}) adopts the fd (fd 0 -> EINVAL). socket-reconnect: end() latches reset on _undestroy. connect() surfaces the sync errno via nm_connect. write-slow: !allowHalfOpen now auto-ends the writable side even when EOF arrives paused-with-buffered-data (the fcntl-blocking theory was wrong — nm_set_nonblock works). remaining 44 are FAILs not timeouts (assorted feature gaps). |
