...
| http2      | 33/165    | 20%  | low      | **MEASURED 2026-07-16.** frame codec + HPACK exist and the round-trip matches the oracle byte-for-byte — the gap is missing surface, not a broken protocol. :method/:authority/:scheme defaults FIXED. setTimeout stubs FIXED (Http2Stream + Http2Session roots; Request/Response delegate). next: writeEarlyHints, and 33 timeouts to triage — each is one live handle + one missing js event (auditor's live sampling) |
