...
| net        | 60/106    | 57%  | high     | **MEASURED 2026-07-17.** pre-session 44 (oom 3, timeout 12). session: **44->60, oom 3->0, timeout 12->2**. Socket DOES extend Duplex already — that blocker is stale. dns-lookup lever DONE. end()-before-connect defers FIN to 'connect'. listen({fd}) adopts the fd (fd 0 -> EINVAL). socket-reconnect: end() latches (_finishing/_prefinished) now reset on _undestroy so 'close' fires on the 2nd connection. remaining 2 timeouts: write-slow (fcntl-variadic blocking), connect-options-port (connect(0) emits no error) |
