...
globalThis.console = {
  ...
  timeEnd(label) { label = label || 'default'; if (!(label in _timers)) { console.warn(`Timer '${label}' does not exist`); return; } const d = Date.now() - _timers[label]; delete _timers[label]; console.log(label + ': ' + d + 'ms'); },
  timeLog(label, ...args) { label = label || 'default'; if (!(label in _timers)) { console.warn(`Timer '${label}' does not exist`); return; } const d = Date.now() - _timers[label]; console.log(label + ': ' + d + 'ms', ...args); },
